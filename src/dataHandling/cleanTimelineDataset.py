...
def train_val_test_split(df: pd.DataFrame, test_size: float = 0.1) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame,
                                                                       pd.DataFrame):
    """
    ...
    val_matchIds = matchIds[test_length:test_length + val_len]
    train_matchIds = matchIds[test_length + val_len:]
...
def check_correct_split(df: pd.DataFrame, train_df, test_df, val_df):
    ...
    all_matchIds = set().update(train_matchIds).update(test_matchIds).update(val_matchIds)  # this removes duplicate
    # matchIds
...
def cleanTimelineDataset():
    ...
    train_df, train_labels, test_df, test_labels = train_val_test_split(df, 0.2)
