...
This evaluation indicates whether the game is a win, loss or a draw, assuming perfect play from both sides.
...
Despite a viewer survey by \citeauthor{claytorRiotGamesAWS2023} \cite{claytorRiotGamesAWS2023} indicating that 94 percent of participants found this graphic useful, there remains a notable lack of publicly available data concerning the graph's methodology and effectiveness.
Addressing this gap, the goal of this thesis is to develop a win prediction model for \acl{lol} both for pre-game and real-time win prediction.
...
\caption{Output of the proprietary win prediction model developed by Riot Games from the final game of the 2022 World championships. The icons at the top of the graph are important events during the game.\cite{LoLEsports}}
...
\end{equation}
where $\mathbf{a}^{(l-1)}$ represents the activation of the last layer, $\mathbf{W}^{(l)}$ the weight matrix, and $\mathbf{b}^{(l)}$ the bias vector of layer $l$.
...
The primary source of data stems from the Riot Games API \cite{RiotDeveloperPortal}, a comprehensive repository of information pertaining to League of Legends gameplay.
...
Below, the pre-game dataset characteristics are presented in more detail, as the smaller in-game dataset is a random sampling from the pre-game dataset.
...
This spike corresponds to the earliest possible conclusion time for a match, as League of Legends prohibits surrendering prior to the 15th minute of gameplay. 
