...
The time at which the data for win prediction is collected is of utmost importance, as shown in Table \ref{tab:related_work_lol}.
...
\citeauthor{bahrololloomiESportsPlayerPerformance2023} \cite{bahrololloomiESportsPlayerPerformance2023} have built a predictor using post-game data from professional matches achieving 86\% accuracy.

\citeauthor{doUsingMachineLearning2021} \cite{doUsingMachineLearning2021} achieved a comparatively high accuracy of 75.1\% while using only pre-game data. 
Using only player-champion winrate and champion mastery points along with features derived off of these they trained a \ac{ann}
Most other authors using pre-game data \cite{costaFeatureAnalysisLeague2021, hitar-garciaMachineLearningMethods2023, whiteScalablePsychologicalMomentum2020, doUsingMachineLearning2021} achieved accuracies of 72\% or less.

An underrepresented area of research is the win prediction during the game.
\Citeauthor{silvaContinuousOutcomePrediction2018} \cite{silvaContinuousOutcomePrediction2018} trained a \acf{rnn} on 7621 games using data from different time intervals between 0-5 min and 20-25 min.
...
\citeauthor{baileyStatisticalLearningEsports} \cite{baileyStatisticalLearningEsports} have used 671 professional matches to train a pre-game predictor with the comparatively high accuracy of 76\%.
\begin{table}[H]
\centering
\begin{longtblr}[
caption = {Comparison of different works on League of Legends win prediction},
label = {tab:related_work_lol},
note{a} = {The exact number of features is unclear},
note{b} = {The exact minute is unclear, but it uses in-game data},
]
{colspec = {*{5}{c}}}
...
\citeauthor{whiteScalablePsychologicalMomentum2020} \cite{whiteScalablePsychologicalMomentum2020} & 87,743 & \TblrNote{a} & pre-game & 0.721 \\
\citeauthor{hitar-garciaMachineLearningMethods2023} \cite{hitar-garciaMachineLearningMethods2023} & 7583 & 26 & pre-game & 0.683 \\

\citeauthor{kimConfidenceCalibratedMOBAGame2020} \cite{kimConfidenceCalibratedMOBAGame2020} & 93875 & 295 & \TblrNote{b} & 0.738 \\
...
\citeauthor{aniVictoryPredictionLeague2019} \cite{aniVictoryPredictionLeague2019} & 1,500 & 97 & post-game & 0.955 \\
\end{longtblr}
\end{table}
...
\begin{tikzpicture}[
   ...
   %Start drawing the thing...    
	...
	%Draw operators	
	%top row
	\node [operator] (mux1) at (1,1.5) {$\times$};
    \node [operator] (add1) at (2,1.5) {$+$};
    %second row
    \node [operator] (mux2) at (-1.5, 0.5) {$\times$};
    \node [operator] (mux3) at (2,  0.5) {$\times$};
    %bottom row
    \node [gt] (sigma1) at (-1, -0.5) {$\sigma$};
    \node [gt] (sigma2) at (1, -0.5) {$\sigma$};
  ...
  % Draw External inputs
    \node[] (ht-1) at (-4,1.5) {$h_{t-1}$};
    \node[] (xt) at (-2.5,-3) {$x_t$};

    % Draw External outputs
    \node[] (out) at (4,1.5) {out};
    \node[] (ht) at (2.5,3) {$h_t$};
    
      % Draw Arrows
    
   \draw [ArrowC2] (ht-1) -- (mux1) -- (add1) -- (out);
   \draw[ArrowC2] (add1) -| (ht);
...
The raw in-game dataset contains 473 features
