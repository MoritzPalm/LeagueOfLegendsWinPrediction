...
The Champion Information feature $\mathbf{x}_c$ is composed of three parts: 

\paragraph{Player-Champion Information}
 containing both general information about the match, including the platform id, the patch or the type of game played (i.e. rated or unrated game) and $36$ columns per player containing information about each individual player.
These include the level of the account, the rank and various stats about the specific selected champion.
Various columns containg different IDs are removed as they contain no relevant information about the game.
The column containing the duration of the game is removed, as this information is not known until the game concludes.
All columns are converted into a floating point number representation to 
